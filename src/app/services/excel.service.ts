...
export class ExcelService {
  ...
  categoria(categri) {
    const ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(categri, {
      header: ['id', 'nombre', 'pago', 'total'],
    });
    let objectMaxLength = [];
  ...
  convoc(convoc: any) {
    ...
    let ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(convoc, {
      header: ['ID', 'fechaInicio', 'fechaFin'],
    });
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
  ...
  piezasPorCategoria(
    ...
    piezasInscritas: any,
    usuariosConPiezasInscritas: any,
    usuariosSinPiezasInscritas: any,
    ordenesPagadas: any,
    ordenesNoPagadas: any
  ) {
    // console.log('pago', ordenesPagadas);
    // console.log('no', ordenesNoPagadas);

    let ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(piezasPorCategoria, {
      header: ['id', 'nombre', 'pago', 'total'],
    });
    let wsPiezasInscritas: XLSX.WorkSheet = XLSX.utils.json_to_sheet(
      piezasInscritas,
      {
        header: [
          '#',
    ...
    let wsUsuariosConPiezasInscritas: XLSX.WorkSheet = XLSX.utils.json_to_sheet(
      usuariosConPiezasInscritas,
      {
        header: [
          '#',
          'ID_USUARIO',
    ...
    let wsUsuariosSinPiezasInscritas: XLSX.WorkSheet = XLSX.utils.json_to_sheet(
      usuariosSinPiezasInscritas,
      {
        header: [
          '#',
          'ID_USUARIO',
    ...
    let wsOrdenesPagadas: XLSX.WorkSheet = XLSX.utils.json_to_sheet(
      ordenesPagadas,
      {
        header: [
          '#',
    ...
    let wsOrdenesNoPagadas: XLSX.WorkSheet = XLSX.utils.json_to_sheet(
      ordenesNoPagadas,
      {
        header: [
          '#',
    ...
    XLSX.utils.book_append_sheet(
      wb,
      wsUsuariosConPiezasInscritas,
      'USUARIOS CON PIEZAS INSCRITAS'
    );
    XLSX.utils.book_append_sheet(
      wb,
      wsUsuariosSinPiezasInscritas,
      'USUARIOS SIN PIEZAS INSCRITAS'
    );
  ...
  reservaciones(reservacionesCollection: any) {
    let ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(reservacionesCollection, {
      header: [],
    });
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
  ...
  reservas(reservacionesCollection: any) {
    let ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(reservacionesCollection, {
      header: [],
    });
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
